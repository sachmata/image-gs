...
class GaussianSplatting2D(nn.Module):
    ...
    def _load_target_images(self, path, downsample_ratio=None):
        self._gt_images, self.input_channels, self.image_fnames, self.bit_depths = (
            load_images(
                load_path=path, downsample_ratio=downsample_ratio, gamma=self.gamma
            )
        )
        self.gt_images = torch.from_numpy(self._gt_images).to(
    ...
    def _separate_and_save_images(self, images, channels, path):
        ...
        for idx, image in enumerate(images_sep, 1):
            ...
            save_image(
                image, f"{path}{suffix}.{self.save_image_format}", gamma=self.gamma
            )
    ...
    def _log_compression_rate(self):
        ...
        for num_channels, bit_depth in zip(self.input_channels, self.bit_depths):
            bytes_uncompressed += float(
                self.gt_images[curr_channel : curr_channel + num_channels].numel()
            ) * (bit_depth / 8.0)
        ...
        self.worklog.info(
            f"Uncompressed: {bytes_uncompressed / 1e3:.2f} KB | {bpp_uncompressed:.3f} bpp | {bppc_uncompressed:.3f} bppc"
        )
        bits_compressed = (
            2 * self.pos_bits
            + 2 * self.scale_bits
    ...
    def _compute_gmap(self):
        ...
        save_image(
            g_norm,
            f"{self.log_dir}/gmap_res-{self.img_h:d}x{self.img_w:d}.{self.save_image_format}",
        )
    ...
    def _compute_smap(self, path):
        ...
        save_image(
            smap,
            f"{self.log_dir}/smap_res-{self.img_h:d}x{self.img_w:d}.{self.save_image_format}",
        )
